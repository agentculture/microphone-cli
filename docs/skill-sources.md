...
microphone-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. microphone-cli
...
#   - consumer-identifying prose: `guildmaster` → `microphone-cli` (NOT
...
If a re-sync would lose a microphone-cli adaptation, lift the change
...
#  `which microphone-cli vendors from guildmaster`).
