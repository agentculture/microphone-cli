"""Smoke tests for the microphone-cli CLI entry point and its verbs."""
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: microphone-cli" in capsys.readouterr().out
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# microphone-cli" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "microphone-cli whoami" in payload["markdown"]
