...
type: command
description: >
  Cross-repo + mesh communication from microphone-cli: file tracked GitHub issues
  ...
  the next step lives outside microphone-cli (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- microphone-cli (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-microphone-cli
