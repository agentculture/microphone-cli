...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope microphone-cli
...
scope:** the default personal scope is **private** (`--scope microphone-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `microphone-cli`). Private records are served only to a recall in the same
