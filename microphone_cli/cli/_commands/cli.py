"""``microphone cli`` — noun grouping CLI-surface introspection.
...
def _no_verb(args: argparse.Namespace) -> int:
    # `microphone cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'microphone cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe this CLI surface.")
