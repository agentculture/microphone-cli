"""``microphone explain <path>...`` — global markdown catalog lookup (stable-contract).

``explain`` is global (not nested under a noun). It takes zero or more path
tokens and resolves them via the catalog in :mod:`microphone_cli.explain`.
...
from microphone_cli.explain import resolve
...
def register(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        "path",
        nargs="*",
        help="Command path tokens; empty = root (same as the bare tool name).",
    )
