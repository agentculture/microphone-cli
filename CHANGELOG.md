...
- **Worktree location convention** in `CLAUDE.md` — every worktree you create
  ...
  `../.worktrees.microphone-cli/<name>/`, one
...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`microphone-cli vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/microphone-cli/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `microphone_cli/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=microphone_cli` (absolute / `.venv` paths were dropped
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/microphone-cli/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: microphone-cli`,
